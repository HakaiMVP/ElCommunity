            ...
            try {
                const { error } = await supabase
                    ...
                    .update({
                        ...
                        equipped_profile_effect: null,
                        equipped_name_color: null,
                        equipped_card_background: null
                    })
                    .eq('id', user.id);
        ...
        showConfirm('Remover Item', `Tem certeza que deseja remover o item "${item.item_id}" do inventário de ${user.username}?`, async () => {
            ...
            try {
                ...
                if (item.is_equipped) {
                    ...
                    if (item.item_type === 'avatar_decoration') updates.equipped_avatar_decoration = null;
                    if (item.item_type === 'profile_effect') updates.equipped_profile_effect = null;
                    if (item.item_type === 'name_color') updates.equipped_name_color = null;
                    if (item.item_type === 'card_background') updates.equipped_card_background = null;
                    if (item.item_type === 'bundle') {
                        updates.equipped_frame = null;
                        updates.equipped_effect = null;
                        updates.equipped_avatar_decoration = null;
                        updates.equipped_profile_effect = null;
                        updates.equipped_name_color = null;
                        updates.equipped_card_background = null;
                    }
    ...
    return createPortal(
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-[#1e1f2b] w-full max-w-2xl rounded-2xl border border-white/10 shadow-2xl overflow-hidden animate-scale-in relative">
                ...
                {userProfile && (userProfile.equipped_frame || userProfile.equipped_effect || userProfile.equipped_avatar_decoration || userProfile.equipped_profile_effect || userProfile.equipped_name_color || userProfile.equipped_card_background) && (
                    <div className="px-6 py-3 bg-purple-500/10 border-b border-white/5 flex items-center justify-between">
                        <div className="text-sm">
                            <span className="text-gray-400 mr-2">Cosméticos Ativos</span>
                        </div>
                        <button
