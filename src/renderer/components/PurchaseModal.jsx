...
const PurchaseModal = ({ item, userStars = 0, onConfirm, onClose, isOpen, discountedPrice }) => {
    const effectivePrice = discountedPrice != null ? discountedPrice : (item?.starsPrice || 0);
    const hasDiscount = discountedPrice != null && discountedPrice < (item?.starsPrice || 0);
    ...
    const handlePurchase = async (method) => {
        ...
        if (method === 'stars') {
            if (userStars < effectivePrice) {
                setStep('error');
                setErrorMsg(`Saldo insuficiente! Faltam ${(effectivePrice - userStars).toLocaleString('pt-BR')} estrelas.`);
                return;
            }
        }
                ...
                <div className="relative p-8 pt-10 text-center z-10">
                    ...
                    {step === 'confirm' && (
                        <>
                            ...
                            {hasDiscount && (
                                <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-3 mb-3 text-center">
                                    <p className="text-green-400 text-xs font-bold">🎉 Desconto aplicado! Você já possui itens deste pacote.</p>
                                </div>
                            )}
                                    ...
                                    <div className="flex flex-col items-end">
                                        {hasDiscount && (
                                            <span className="text-xs text-gray-500 line-through">
                                                {item.starsPrice.toLocaleString()}
                                            </span>
                                        )}
                                        <span className={`font-bold flex items-center gap-1 ${hasDiscount ? 'text-green-400' : 'text-white'}`}>
                                            {effectivePrice.toLocaleString()} <FaStar className="text-xs text-yellow-400" />
                                        </span>
                                    </div>
                                </button>
