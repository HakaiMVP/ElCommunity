...
const ViewUserProfileModal = ({ isOpen, onClose, targetUserId, fromAdminDashboard = false }) => {
    ...
    const cosmetics = getCosmeticsForUser(profile);

    return createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
            <div className={`w-full max-w-sm rounded-[2rem] border border-white/10 shadow-2xl overflow-hidden relative ${cosmetics.cardBackground ? cosmetics.cardBackground : 'bg-[#1e1f2b]'}`}>
                {/* Card Background Color overlay if not image */}
                {!cosmetics.cardBackground?.includes('bg-[url') && <div className="absolute inset-0 bg-[#1e1f2b] -z-20"></div>}
                <div className="absolute inset-0 bg-black/40 z-[-10] pointer-events-none"></div>

                {/* Close Button */}
                <button
                    onClick={onClose}
                    className="absolute top-4 right-4 z-20 text-white/50 hover:text-white bg-black/20 hover:bg-black/50 p-2 rounded-full transition-all backdrop-blur-md"
                >
                    <FaTimes size={16} />
                </button>
                ...
                {loading ? (
                    ...
                    <>
                        {/* Banner & Avatar Area */}
                        <div
                            className="h-32 bg-[#181922] relative"
                            style={{
                                backgroundColor: profile?.banner_color || '#1e1f2b',
                            }}
                        >
                            {/* Cover Image */}
                            {profile?.cover_url && (
                                <img src={profile.cover_url} alt="Cover" className="absolute inset-0 w-full h-full object-cover opacity-80 mix-blend-overlay" />
                            )}
                            ...
                            <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 z-10">
                                <div className={`w-28 h-28 rounded-full p-1 bg-[#1e1f2b] relative flex items-center justify-center shadow-2xl ${cosmetics.avatarBorder}`}>
                        ...
                        <div className="pt-14 pb-8 px-6 text-center relative z-10">
                            <h2 className={`text-2xl font-black flex items-center justify-center gap-2 drop-shadow-md tracking-tight ${cosmetics.nameColor}`}>
