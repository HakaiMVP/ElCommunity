...
const PostCard = ({ post, onDelete, canComment, isModerator, showCommunityContext }) => {
    ...
    const fetchComments = async () => {
        const { data } = await supabase
            .from('community_comments')
            .select('*, profiles:community_comments_author_fkey(username, avatar_url, equipped_frame, equipped_effect, equipped_avatar_decoration, equipped_profile_effect, equipped_name_color, equipped_card_background)')
            .eq('post_id', post.id)
    ...
    const postCosmetics = getCosmeticsForUser(post.profiles || {});
    ...
    return (
        <div className="bg-gray-800 rounded-xl border border-gray-700 mb-4 overflow-hidden shadow-md animate-fade-in group relative">
            {/* Header */}
            <div className={`p-4 flex items-center justify-between relative z-10 ${postCosmetics.cardBackground || ''}`}>
                <div className="flex items-center gap-3">
                    <div
                        className={`w-10 h-10 rounded-full bg-gray-700 overflow-hidden cursor-pointer hover:opacity-80 transition-opacity relative ${postCosmetics.avatarBorder || ''}`}
                        onClick={() => handleOpenProfile(post.user_id)}
                    >
                        {post.profiles?.avatar_url ? <img src={post.profiles.avatar_url} className="w-full h-full object-cover relative z-10" /> :
                            <div className="w-full h-full flex items-center justify-center text-gray-500 font-bold relative z-10">{post.profiles?.username?.[0]}</div>}
                    </div>
                    <div>
                        <div className="flex items-center gap-2">
                            <p
                                className={`font-bold text-sm cursor-pointer hover:underline ${postCosmetics.nameColor || 'text-white'}`}
                                onClick={() => handleOpenProfile(post.user_id)}
                            >
                        ...
                        {comments.length === 0 ? <p className="text-gray-500 text-sm text-center">Nenhum comentário.</p> :
                            (() => {
                                ...
                                return topLevel.map(c => {
                                    const topCosmetics = getCosmeticsForUser(c.profiles || {});
                                    return (
                                        <div key={c.id} className={`border-b border-white/5 last:border-0 pb-3 last:pb-0 ${topCosmetics.cardBackground || ''}`}>
                                            {/* Top-level comment */}
                                            <div className="flex gap-3 animate-fade-in-up">
                                                <div
                                                    className={`w-8 h-8 rounded-full bg-gray-700 overflow-hidden shrink-0 cursor-pointer hover:opacity-80 transition-opacity relative ${topCosmetics.avatarBorder || ''}`}
                                                    onClick={() => handleOpenProfile(c.user_id)}
                                                >
                                                    {c.profiles?.avatar_url ? <img src={c.profiles.avatar_url} className="w-full h-full object-cover relative z-10" /> :
                                                        <div className="flex items-center justify-center w-full h-full text-xs font-bold relative z-10">{c.profiles?.username?.[0]?.toUpperCase()}</div>}
                                                </div>
                                                <div className="bg-gray-800 p-3 rounded-lg rounded-tl-none flex-1">
                                                    <div className="flex justify-between items-start mb-1">
                                                        <p
                                                            className={`text-xs font-bold mb-1 cursor-pointer hover:underline ${topCosmetics.nameColor || 'text-gray-300'}`}
                                                            onClick={() => handleOpenProfile(c.user_id)}
                                                        >
                                                            {c.profiles?.username}
                                                        </p>
                                            ...
                                            {threadMap[c.id]?.map(r => {
                                                const replyCosmetics = getCosmeticsForUser(r.profiles || {});
                                                return (
                                                    <div key={r.id} className={`flex gap-3 animate-fade-in-up ml-8 mt-2 p-1 rounded-lg ${replyCosmetics.cardBackground || ''}`}>
                                                        <div
                                                            className={`w-7 h-7 rounded-full bg-gray-700 overflow-hidden shrink-0 cursor-pointer hover:opacity-80 transition-opacity relative ${replyCosmetics.avatarBorder || ''}`}
                                                            onClick={() => handleOpenProfile(r.user_id)}
                                                        >
                                                            {r.profiles?.avatar_url ? <img src={r.profiles.avatar_url} className="w-full h-full object-cover relative z-10" /> :
                                                                <div className="flex items-center justify-center w-full h-full text-[10px] font-bold relative z-10">{r.profiles?.username?.[0]?.toUpperCase()}</div>}
                                                        </div>
                                                        <div className="bg-gray-800/70 p-2.5 rounded-lg rounded-tl-none flex-1 border-l-2 border-purple-500/30">
                                                            <div className="flex justify-between items-start mb-1">
                                                                <p
                                                                    className={`text-xs font-bold mb-1 cursor-pointer hover:underline ${replyCosmetics.nameColor || 'text-gray-300'}`}
                                                                    onClick={() => handleOpenProfile(r.user_id)}
                                                                >
                                                                    {r.profiles?.username}
                                                                </p>
