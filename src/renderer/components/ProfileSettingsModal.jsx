...
const ProfileSettingsModal = ({ isOpen, onClose }) => {
    ...
    const handleSave = async () => {
        try {
            ...
            // 1. Update profiles table first (this is our source of truth)
            ...
            // 2. Update auth metadata (this triggers auth state changes)
            ...
            if (authError) {
                console.warn('Auth metadata update failed, but profile was saved:', authError);
            }

            // 3. Sync global AuthContext state
            if (refreshProfile) {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            // Don't overwrite local state if we are currently saving or model is closed
            if (user && isOpen && !saving) {
                const { data, error } = await supabase
                    .from('profiles')
    ...
    const cosmetics = getCosmeticsForUser(userProfile || {});
                    ...
                    <div className="max-w-4xl mx-auto w-full">
                        {activeTab === 'profile' ? (
                            <div className="space-y-12 pb-24">
                                ...
                                <div className={`rounded-3xl pb-8 border border-white/5 shadow-2xl relative overflow-hidden ${cosmetics.cardBackground ? cosmetics.cardBackground : 'bg-[#1e1f2b]'}`}>
                                    ...
                                    <div className="relative mt-28 mb-8 flex items-end justify-between px-8">
                                        <div className="relative group -mt-24">
                                            <div className={`w-[120px] h-[120px] rounded-full bg-[#161721] flex items-center justify-center shadow-2xl overflow-hidden z-10 relative ${cosmetics.avatarBorder} ring-[6px] ring-[#1e1f2b]`}>
                                    ...
                                    <div className="space-y-6 px-8">
                                        ...
                                        <div className="pt-4 border-t border-white/5">
                                            <div className="bg-[#161721] border border-white/5 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-lg">
                                                ...
                                                <div className={`px-6 py-3 rounded-xl shadow-inner border transition-all duration-500 overflow-hidden relative ${cosmetics.profileEffect ? 'border-transparent bg-transparent' : 'bg-[#0e0f14] border-white/5'}`}>
                                                    {cosmetics.profileEffect && <div className="absolute inset-0 bg-[#0e0f14]/80 z-[-1]"></div>}
                                                    <span className={`font-bold text-xl relative z-10 ${cosmetics.nameColor}`}>
