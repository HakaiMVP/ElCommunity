...
const Chat = () => {
    ...
    const fetchSocials = async () => {
        ...
        try {
            ...
            const { data: profileData, error: profileError } = await supabase
                .from('profiles')
                .select('id, username, avatar_url, display_id, last_seen, presence, cover_url, equipped_frame, equipped_effect, equipped_avatar_decoration, equipped_profile_effect, equipped_name_color, equipped_card_background, bio')
                .in('id', friendIds);
    ...
    return (
        <div className="flex h-[calc(100vh-2rem)] text-white animate-fade-in gap-6 p-6">
                    ...
                    <div className="p-2 space-y-1">
                        {(activeTab === 'friends' ? friends : marketChats)
                            ...
                            .map(chat => {
                                ...
                                const chatCosmetics = getCosmeticsForUser(chat.profiles || {});

                                const nameColor = chatCosmetics.nameColor || 'text-gray-300 group-hover:text-white';
                                const avatarBorder = chatCosmetics.avatarBorder || '';

                                const baseBg = isActive ? (chatCosmetics.cardBackground || 'bg-[#2d2f3b] border-purple-500/50 shadow-md') : 'border-transparent hover:bg-white/5';
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full group flex items-center gap-3 p-3 rounded-xl transition-colors relative border overflow-hidden ${baseBg}`}
                                    >
                                        {chatCosmetics.profileEffect && (
                                        ...
                                        <div className={`relative z-10 w-10 h-10 rounded-full bg-gradient-to-br ${isMarket ? 'from-green-600 to-teal-700' : 'from-blue-600 to-purple-700'} flex items-center justify-center font-bold text-sm text-white overflow-hidden flex-shrink-0 ${avatarBorder}`}>
                                            {chat.avatar_url ? <img src={chat.avatar_url} alt="" className="w-full h-full object-cover relative z-10" /> : (isMarket ? '$' : (chat.friendName || '?').slice(0, 1).toUpperCase())}
                                        </div>
            ...
            {/* Chat Window Wrapper */}
            <div className={`flex-1 rounded-2xl flex flex-col overflow-hidden relative border border-white/5 bg-[#1e1f2b]`}>
                ...
                <div className="flex-1 flex flex-col relative z-10 w-full h-full">
                    {activeChat ? (
                        (() => {
                            const activeChatCosmetics = getCosmeticsForUser(activeChat.profiles || {});

                            const headerNameColor = activeChatCosmetics.nameColor || 'text-white';
                            const headerAvatarBorder = activeChatCosmetics.avatarBorder || '';

                            return (
                                <>
                                    {/* Header */}
                                    <div className={`p-4 border-b border-white/5 bg-[#161721] flex items-center justify-between shadow-md z-10 flex-shrink-0 relative overflow-hidden ${activeChatCosmetics.cardBackground || ''}`}>
                                        {activeChatCosmetics.profileEffect && (
                                            <div className="absolute inset-0 overflow-hidden pointer-events-none z-0 opacity-80 backdrop-blur-sm mix-blend-screen bg-black/50">
                                                {activeChatCosmetics.profileEffect()}
                                            </div>
                                        )}
                                        <div className="flex items-center gap-3 cursor-pointer hover:bg-white/5 px-2 py-1 rounded-lg transition-colors relative z-10" onClick={() => setShowProfile(true)}>
                                            <div className={`w-10 h-10 rounded-full bg-gradient-to-br from-blue-600 to-purple-700 flex items-center justify-center font-bold overflow-hidden flex-shrink-0 ${headerAvatarBorder}`}>
                                                {activeChat.avatar_url ? <img src={activeChat.avatar_url} alt="" className="w-full h-full object-cover relative z-10" /> : (activeChat.friendName || '?').slice(0, 1).toUpperCase()}
                                            </div>
                                            <div>
