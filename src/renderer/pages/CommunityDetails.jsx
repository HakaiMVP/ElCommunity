    ...
    const fetchCommunityDetails = async () => {
        ...
        try {
            ...
            const { data: memData } = await supabase
                .from('community_members')
                .select('*, profiles(username, avatar_url, equipped_frame, equipped_effect, equipped_avatar_decoration, equipped_profile_effect, equipped_name_color, equipped_card_background)')
                .eq('community_id', id)
                .eq('status', 'approved')
            ...
            <div className="bg-[#1e2029] rounded-xl p-6 border border-gray-800">
                ...
                {activeTab === 'members' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {members.map(member => {
                            const memberCosmetics = getCosmeticsForUser(member.profiles || {});
                            return (
                                <div key={member.id} className={`flex items-center gap-4 p-4 bg-gray-800/40 rounded-xl hover:bg-gray-800/60 transition-all duration-300 group relative overflow-hidden border border-white/5 hover:border-white/10 ${memberCosmetics.cardBackground || ''}`}>
                                    {memberCosmetics.profileEffect && (
                                    ...
                                    <div className={`w-12 h-12 rounded-full bg-gray-700/50 overflow-hidden relative z-10 shrink-0 ${memberCosmetics.avatarBorder || ''}`}>
                                        {member.profiles?.avatar_url ? (
                                            <img src={member.profiles.avatar_url} alt="" className="w-full h-full object-cover relative z-10" />
                                        ) : (
                                            <div className="w-full h-full flex items-center justify-center text-gray-500 font-bold text-lg relative z-10">
                                                {member.profiles?.username?.[0]?.toUpperCase()}
                                            </div>
                                        )}
                                    </div>
                                    <div className="flex-1 relative z-10 flex flex-col items-start justify-center min-w-0">
                                        <p className={`font-bold text-base truncate ${memberCosmetics.nameColor || 'text-gray-100'}`}>
                    ...
                    <div>
                        ...
                        {requests.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {requests.map(req => {
                                    const reqCosmetics = getCosmeticsForUser(req.profiles || {});
                                    return (
                                        <div key={req.id} className="flex items-center justify-between p-4 bg-gray-800 rounded-xl border border-gray-700">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-12 h-12 rounded-full bg-gray-700 overflow-hidden relative ${reqCosmetics.avatarBorder || ''}`}>
                                                    {req.profiles?.avatar_url ? (
                                                        <img src={req.profiles.avatar_url} alt="" className="w-full h-full object-cover relative z-10" />
                                                    ) : (
                                                        <div className="w-full h-full flex items-center justify-center text-gray-500 font-bold relative z-10">
                                                            {req.profiles?.username?.[0]?.toUpperCase()}
                                                        </div>
                                                    )}
                                                </div>
                                                <div>
                                                    <p className={`font-bold ${reqCosmetics.nameColor || 'text-white'}`}>{req.profiles?.username}</p>
