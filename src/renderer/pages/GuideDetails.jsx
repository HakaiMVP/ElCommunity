...
const GuideDetails = () => {
            ...
            if (error) {
                ...
                try {
                    const { data: profileData } = await supabase
                        .from('profiles')
                        .select('*, equipped_frame, equipped_effect, equipped_avatar_decoration, equipped_profile_effect, equipped_name_color, equipped_card_background') // Fetch all cosmetics
    ...
    const guideCosmetics = getCosmeticsForUser(guide.profiles || {});

    return (
        <div className="p-6 md:p-10 text-white animate-fade-in max-w-5xl mx-auto pb-20 relative">
            {guideCosmetics.profileEffect && (
                <div className="absolute inset-0 overflow-hidden pointer-events-none z-0 opacity-80 backdrop-blur-sm mix-blend-screen bg-black/50 rounded-[40px]">
                    {guideCosmetics.profileEffect()}
                </div>
            )}
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-4">
                        <div className="flex items-center gap-4">
                            <div
                                ...
                                className={`w-12 h-12 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center font-bold text-xl shadow-lg overflow-hidden flex-shrink-0 cursor-pointer hover:scale-105 transition-transform border-2 border-transparent hover:border-purple-500 relative ${guideCosmetics.avatarBorder || ''}`}
                            >
                                {getUserAvatar(guide.profiles) ? (
                                    <img src={getUserAvatar(guide.profiles)} alt="" className="w-full h-full object-cover relative z-10" />
                                ) : (
                                    <span className="relative z-10">{formatUserName(guide.profiles, guide.author_name).charAt(0).toUpperCase()}</span>
                                )}
                            </div>
                            <div>
                                <div className="flex items-center flex-wrap gap-2">
                                    <p
                                        onClick={() => openProfileModal(guide.author_id)}
                                        className={`font-bold text-lg flex items-center gap-2 cursor-pointer transition-colors ${guideCosmetics.nameColor || 'text-white hover:text-purple-400'}`}
                                    >
